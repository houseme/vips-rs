...
- Local development resolves sibling `vips-sys` via `path = "../vips-sys"` (versioned for crates.io).
...
- Owned vs borrowed constructor guidance and lifetime pitfalls in both READMEs.
...
- Dropped the `rust-version` field; edition remains `2024` (Rust ≥ 1.85).
